...
@Composable
fun CustomPreviewTxInput(model: ReviewTxModel) {
    ...
    Box {
        Column (
            ...
            for (utxoPoint in allTxOutPoints!!.sortedWith(MyTransactionOutPointAmountComparator())) {
                DisplayUtxoOutPoint(
                    model = model,
...
fun DisplayUtxoOutPoint(
    ...
    val sendType = model.impliedSendType.observeAsState()
    ...
    Row(
        ...
        if (sendType.value!!.isCustomSelection) {
            Checkbox(
                checked = selected,
